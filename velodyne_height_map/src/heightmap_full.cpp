...
HeightMap::HeightMap(ros::NodeHandle node, ros::NodeHandle priv_nh)
{
  ...
  // Set up publishers  
  obstacle_publisher_ = node.advertise<VPointCloud>("/carina/sensor/lidar/velodyne_obstacles_full",1);
  // clear_publisher_ = node.advertise<VPointCloud>("velodyne_clear",1);  

  // subscribe to Velodyne data points
  velodyne_scan_ = node.subscribe("/carina/sensor/lidar/point_cloud", 1,
...
// void HeightMap::constructGridClouds(const VPointCloud::ConstPtr &scan, unsigned npoints, size_t &obs_count)
